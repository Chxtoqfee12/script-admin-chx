...
local currentValues = {
	TPWalkEnabled = false,
	TPWalkSpeed = 1, -- 1 = ปกติ
	JumpBoostEnabled = false,
	JumpPower = 50, -- ความแรงกระโดด
}
...
local Section = Tab:CreateSection("TP Walk & Jump")
...
Tab:CreateSlider({
	...
	Increment = 0.1,
	Suffix = "x",
	CurrentValue = currentValues.TPWalkSpeed,
	Callback = function(value)
		currentValues.TPWalkSpeed = value
	end
})
...
Tab:CreateToggle({
	...
	CurrentValue = false,
	Callback = function(v)
		currentValues.JumpBoostEnabled = v
	end
})
...
Tab:CreateSlider({
	...
	Callback = function(v)
		currentValues.JumpPower = v
	end
})

------------------------------------------------------------
-- 🌈 Smooth Jump (Tween ขึ้น + Gravity ลง)
------------------------------------------------------------
local jumping = false

humanoid.StateChanged:Connect(function(_, state)
	if state == Enum.HumanoidStateType.Jumping and currentValues.JumpBoostEnabled and not jumping then
		jumping = true

		local jumpHeight = currentValues.JumpPower / 2.5 -- ค่าความสูงสัมพันธ์กับ JumpPower
		local startCFrame = hrp.CFrame

		-- ปิดแรงกระโดดเริ่มต้น
		humanoid:ChangeState(Enum.HumanoidStateType.Freefall)

		-- Tween เฉพาะตอนขึ้น (Smooth)
		local tweenUp = TweenService:Create(
			hrp,
			TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
			{ CFrame = startCFrame + Vector3.new(0, jumpHeight, 0) }
		)
		tweenUp:Play()

		-- หลังจาก Tween เสร็จให้ Gravity ดึงลงเอง
...
------------------------------------------------------------
-- 🚶 ระบบ TP Walk (MoveDirection)
------------------------------------------------------------
RunService.Heartbeat:Connect(function()
	if currentValues.TPWalkEnabled and humanoid and hrp and humanoid.MoveDirection.Magnitude > 0 then
		local moveAmount = (currentValues.TPWalkSpeed / 16) * 1.5
		hrp.CFrame = hrp.CFrame + (humanoid.MoveDirection * moveAmount)
	end
end)

------------------------------------------------------------
-- ♻️ รีเซ็ตเมื่อ Respawn
------------------------------------------------------------
