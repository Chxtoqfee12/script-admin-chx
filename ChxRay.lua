...
local invisRunning = false
local IsInvis = false
local Character, InvisibleCharacter, Highlight
local Depth = -15 -- ค่าความลึกเริ่มต้น

-- ฟังก์ชัน Invisible
local function ToggleInvisible(state)
    if state and not invisRunning then
        invisRunning = true
        ...
        InvisibleCharacter = Character:Clone()
        InvisibleCharacter.Parent = workspace
        InvisibleCharacter.Name = "Fake_" .. LocalPlayer.Name

        -- ลบ Script ที่ซ้ำกันในโคลน
        for _, v in pairs(InvisibleCharacter:GetDescendants()) do
        ...
        -- เพิ่ม Highlight ให้ร่างจริงใต้ดิน
        ...
        local fakeHrp = InvisibleCharacter:WaitForChild("HumanoidRootPart")

        hrp.CFrame = fakeHrp.CFrame * CFrame.new(0, Depth, 0)

        -- ซิงค์การเคลื่อนไหว
        RunService.RenderStepped:Connect(function()
            if not invisRunning then return end
            if Character and InvisibleCharacter then
                local hrp = Character:FindFirstChild("HumanoidRootPart")
                local fakeHrp = InvisibleCharacter:FindFirstChild("HumanoidRootPart")
                if hrp and fakeHrp then
                    hrp.CFrame = fakeHrp.CFrame * CFrame.new(0, Depth, 0)
                end
            end
        end)
...
-- ✅ เพิ่มใน UI
Tab:CreateToggle({
    Name = "Invisible V2 (ลงใต้ดิน + Clone)",
    CurrentValue = false,
    Flag = "InvisibleV2",
    Callback = function(state)
...
-- ✅ สไลด์ปรับความลึกของตัวจริง
Tab:CreateSlider({
    Name = "ปรับความลึกใต้ดิน",
    Range = {-50, -1}, -- ค่าลบ = ลงลึก, -50 = ลึกสุด
    Increment = 1,
    Suffix = " Studs",
    CurrentValue = Depth,
    Flag = "DepthSlider",
